...
export class ReimbursementHttpService {
  ...
  addNewReimbursementRequest(reimbursement: Reimbursement): Observable<Reimbursement> {
    ...
    return this.http.post<Reimbursement>("http://localhost:4041/api/addNewReimbursement",JSON.stringify(reimbursement));
  }
  fetchAllReimbursements(): Observable<Reimbursement[]> {
    return this.http.get<Reimbursement[]>("http://localhost:4041/api/getAllReimbursments");
  }
  resolveReimbursement(reimbursement: Reimbursement): Observable<Reimbursement[]>{
    ...
    return this.http.post<Reimbursement[]>("http://localhost:4041/api/resolveReimbursment",JSON.stringify(reimbursement));
  }
  fetchAllReimbursementsByStatus(status: string): Observable<Reimbursement[]>{
    return this.http.get<Reimbursement[]>("http://localhost:4041/api/getAllReimbursments/"+status);
  }

  fetchAllReimbursementsOfUser(userId: number): Observable<Reimbursement[]>{
    return this.http.get<Reimbursement[]>("http://localhost:4041/api/getEmployeeReimbursement/"+userId);
  }

  fetchAllReimbursementsOfUserByStatus(userId:number, status:string): Observable<Reimbursement[]>{
    return this.http.get<Reimbursement[]>("http://localhost:4041/api/getOwnReimbursementByStatus/"+userId+"/"+status);
  }
}
